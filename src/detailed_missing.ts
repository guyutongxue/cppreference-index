export const MISSING_ENUMERATORS = [
  ...
  // https://en.cppreference.com/w/cpp/locale/codecvt_mode
  {
    ...
    description: "assume the input is in little-endian byte order (applies to UTF-16 input only, the default is big-endian)"
  },
  {
    name: "std::consume_header",
    description: "consume the byte order mark, if present at the start of input sequence, and (in case of UTF-16), rely on the byte order it specifies for decoding the rest of the input"
  },
  {
    name: "std::generate_header",
    description: "output the byte order mark at the start of the output sequence"
  },
...
export const MISSING_HELPERS = [
  // <iterator>
  {
    ...
    description: "used to prevent iterators and sentinels that can be subtracted but do not actually model sized_sentinel_for from satisfying the concept."
  },
  // <ranges>
  {
    name: "std::ranges::disable_sized_range",
    type: "variableTemplate" as const,
    description: "allows use of range types that provide a size function (either as a member or as a non-member) but do not in fact model sized_range"
  },
  {
  ...
  },
  {
    ...
    type: "class" as const,
    description: "deriving from std::view_base enables range types to model std::view"
  },
  {
...
export const NAMES_WITH_WRONG_TYPE = [
  ...
  {
    name: "std::in_place_type",
    type: "variableTemplate" as const,
  },
  {
    name: "std::in_place_index",
    type: "variableTemplate" as const,
  },
  {
    name: "std::in_place_t",
  ...
  {
    name: "std::ranges::chunk_by_view",
    type: "class" as const,
  },
  {
    name: "std::ranges::chunk_view",
    type: "class" as const,
  },
  {
    name: "std::ranges::slide_view",
    type: "class" as const,
  },
  {
